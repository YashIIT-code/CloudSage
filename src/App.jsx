  ...
  const runScan = useCallback((raw) => {
    ...
    const step = async () => {
      ...
      if (i < SCAN_STEPS.length) setTimeout(step, 260);
        ...
        try {
          // Map parsed CSV/JSON rows into CloudResource shape for new backend
          const payload = {
            resources: raw.map(r => {
              const provLower = String(r.provider).toLowerCase();
              const typeLower = String(r.type).toLowerCase();
              return {
                resource_name: String(r.resource_id || r.resource_name || "unknown"),
                type: typeLower.includes("database") ? "database"
                  : typeLower.includes("storage") ? "storage"
                    : typeLower.includes("container") ? "container" : "vm",
                provider: ["aws", "gcp", "azure"].find(p => provLower.includes(p)) || "aws",
          ...
          if (!res.ok) {
            ...
            throw new Error(`API ${res.status}: ${errTx}`);
          }

          const data = await res.json();

          // Map the new AnalysisResponse into the shape the dashboard expects
          const mappedResources = (data.breakdown || []).map((b, idx) => {
            const original = raw[idx] || {};
            const ineffs = (data.inefficiencies || []).filter(
              inf => inf.resource_name === b.resource_name
            );
            const topIneff = ineffs[0] || {};
            const isIdle = ineffs.some(inf => inf.issue.toLowerCase().includes("idle") || inf.issue.toLowerCase().includes("auto-shutdown"));
            const isOver = ineffs.some(inf => inf.issue.toLowerCase().includes("oversized") || inf.issue.toLowerCase().includes("downsize") || inf.issue.toLowerCase().includes("cpu"));
            const isBigStore = ineffs.some(inf => inf.issue.toLowerCase().includes("storage") || inf.issue.toLowerCase().includes("cool tier"));
            const noScale = ineffs.some(inf => inf.issue.toLowerCase().includes("reserved"));
            const maxSev = ineffs.reduce((m, inf) => {
              if (inf.severity === "high") return "High";
              if (inf.severity === "medium" && m !== "High") return "Medium";
              return m;
            }, "Low");
            ...
            return {
              ...
              provider: b.provider,
              type: b.type,
              region: original.region || "us-east-1",
              cpu_avg: original.cpu_avg || 0,
              mem_avg: original.mem_avg || 0,
              storage_gb: original.storage_gb || 0,
              monthly_cost: b.monthly_cost,
              hours_idle: original.hours_idle || 0,
              savings: b.saving,
              risk: maxSev,
              isIdle,
              isOver,
              isBigStore,
              noScale,
              co2: Math.round(b.saving * 0.015 * 10) / 10,
              actions: ineffs.length > 0
                ? ineffs.map(inf => ({
                  icon: inf.severity === "high" ? "🛑" : inf.severity === "medium" ? "⚠️" : "💡",
                  text: inf.issue,
                  impact: inf.severity.charAt(0).toUpperCase() + inf.severity.slice(1),
                }))
                : [{ icon: "✅", text: "Well-optimized — schedule monthly review", impact: "Low" }],
              cliCmds: (data.recommendations || []).map(r => r.cli_command).filter(Boolean),
              terraformFix: (data.recommendations || []).map(r => r.terraform_snippet).filter(Boolean),
            };
          });

          setAnalyzed(mappedResources);
          setForecast(buildForecast(mappedResources));

          if (data.analysis) {
            setChat(c => [...c, { role: "ai", text: `**ARIA Executive Summary:**\n${data.analysis}` }]);
          }
          if (data.recommendations && data.recommendations.length > 0) {
            setAiPlan(data.recommendations.map((r, idx) => ({ week: `Target ${idx + 1}`, action: r.title })));
          }

          setPhase("ready");
        } catch (e) {
          console.error("Analysis error:", e);
          alert("Error communicating with Python backend: " + e.message);
          setPhase("upload");
        }
      }
    };
