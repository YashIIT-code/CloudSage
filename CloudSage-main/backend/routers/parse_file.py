"""
CloudSage – /api/parse-file router.
Accepts uploaded CSV / JSON files and returns parsed CloudResource list.
"""

from fastapi import APIRouter, HTTPException, UploadFile, File

from backend.models.schemas import CloudResource
from backend.services.file_parser import parse_file, validate_file
...
async def parse_uploaded_file(file: UploadFile = File(...)):
    ...
    # Validate extension & approximate size
    ...
    if not resources:
        raise HTTPException(status_code=422, detail="No resources found in file. Ensure it has rows/records.")
